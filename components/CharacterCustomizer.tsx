...
export default function CharacterCustomizer({open,onOpenChange,value,onChange,completedQuizCount,totalQuizCount,onEquipRide}:Props){
 ...
 <div className={styles.toggleLayout}><CharacterToggle value={tab} onChange={setTab} label="Customize" options={[{value:'character',label:'Character'},{value:'rides',label:'Balls & rides'}]}/></div>
 <div className={styles.layout}><div className={styles.previewColumn}>{tab==='character'?<CharacterPreview open={open} value={value}/>:<div className={styles.equipmentPreview} data-equipment-preview={previewItem.id}><StorePreview item={previewItem} src={previews[previewItem.id]}/></div>}
