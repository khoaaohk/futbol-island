...
import timeline from '@/public/stories/films/futsl/timeline.json';
const CAPTIONS=timeline.cues.map(cue=>[cue.start,cue.text] as [number,string]);
export default function LoveFutslFilm({onClose,onFinish,embedded: _embedded=false,origin}:{onClose:()=>void;onFinish:()=>void;embedded?:boolean;origin?:{x:number;y:number;size:number;height:number;radius:string;color:string}}){
 ...
 const storySource=()=>{const ratio=window.innerWidth/window.innerHeight;return '/stories/films/futsl/'+(ratio<.75?'portrait':ratio<1.34?'square':'landscape')+'.mp4?v=love-futsl-7-full-court';};
 ...
 const callout=[...timeline.cues].reverse().find(cue=>time>=cue.start)?.title;
 return <dialog ref={dialog} className={`${styles.dialog} ${entering?styles.entering:''} ${exiting?styles.exiting:''}`} aria-labelledby={uid} data-story-view data-story-complete={completed} onCancel={e=>{e.preventDefault();e.stopPropagation();dismiss();}}>
  ...
  <video ref={video} className={styles.film} preload="none" playsInline muted={muted} aria-label="Love Futsl: smaller court, bigger game" aria-describedby={uid+'-caption'} onTimeUpdate={e=>setTime(e.currentTarget.currentTime)} onPlay={()=>{document.dispatchEvent(new CustomEvent('fi2-video-play',{detail:uid}));release.current??=holdVideoPlayback();setPlaying(true);}} onPause={()=>{setPlaying(false);stop();}} onEnded={()=>{setPlaying(false);setCompleted(true);stop();}} onError={()=>{setPlaying(false);setError(true);stop();}}/>
  <header className={`${styles.top} ${local.header}`}><h2 id={uid}>Love Futsl</h2><DoneButton onDone={dismiss}/></header>
  {!started&&<picture className={styles.poster}><source media="(min-aspect-ratio: 3/4) and (max-aspect-ratio: 4/3)" srcSet="/stories/films/futsl/poster-square.jpg?v=love-futsl-7-full-court"/><source media="(orientation: portrait)" srcSet="/stories/films/futsl/poster-portrait.jpg?v=love-futsl-7-full-court"/><img src="/stories/films/futsl/poster-landscape.jpg?v=love-futsl-7-full-court" alt=""/></picture>}
  ...
  <div role="region" className={styles.bottom} aria-label="Story playback and captions">
   ...
   <div className={styles.captionArea}><p id={uid+'-caption'} className={styles.caption}>{completed&&time>timeline.duration-.2?'Scan early. Stay involved. Find your next pass.':!started?'Smaller court. Bigger game.':([...CAPTIONS].reverse().find(([start])=>time>=start)?.[1])}</p></div>
