...
export default function LiveArcadeMatch({onExit}:{onExit:()=>void}){
 ...
 const changePhase=(next:typeof phase)=>{release();phaseRef.current=next;setPhase(next);if(next==='playing'){recordExploreActivity('arcade');host.current?.focus({preventScroll:true});}};
 ...
 useEffect(()=>{
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio,window.matchMedia('(pointer:coarse)').matches?1.25:1.75));renderer.setClearColor('#527253');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.1;node.appendChild(renderer.domElement);
  ...
  sim.resetMatch();sim.enableUser('gold');scene.add(new T.HemisphereLight('#fff6de','#354b39',2.4));const light=new T.DirectionalLight('#fff0cd',2.4);light.position.set(venue.x-20,100,venue.z+30);scene.add(light);
  ...
  const hidden=()=>{if(document.hidden)blur();last=performance.now();};
  ...
  const animate=(now:number)=>{if(disposed)return;frame=requestAnimationFrame(animate);if(document.hidden){last=now;return;}if(now-lastDraw<1000/30)return;lastDraw=now;const dt=Math.min(.05,(now-last)/1000);last=now;const active=phaseRef.current==='playing',k=keys.current,l=local.current;
