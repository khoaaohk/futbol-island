...
export default function Museum({open,onOpenChange}:{open:boolean;onOpenChange:(open:boolean)=>void}){
 ...
 return <dialog ref={dialog} className={styles.dialog} aria-labelledby="museum-title" onCancel={e=>{e.preventDefault();onOpenChange(false);}} onClick={e=>{if(e.target===e.currentTarget)onOpenChange(false);}} onKeyDown={e=>e.stopPropagation()} onKeyUp={e=>e.stopPropagation()}>
  <section className={`${styles.panel} ${shell.shell} `}><header className={shell.header}><div><h2 id="museum-title">History Museum</h2></div><DoneButton ref={close} onDone={()=>onOpenChange(false)}/></header><div className={shell.body}>
   <div className={styles.details}><span className={styles.badge}>Coming soon</span><h3>The story of football</h3><p>Discover the clubs, players and moments that shaped the game.</p></div>
  </div></section>
 </dialog>;
}
