...
import narrationTiming from '@/lib/paths/gritNarrationTiming.json';
import gritScript from '@/lib/paths/gritScript.json';

// Keep responsive labels on the same visual beats after offline narration retiming.
function storyTime(mediaTime:number){
 const next=narrationTiming.findIndex(point=>point.media>mediaTime);
 if(next<0)return narrationTiming[narrationTiming.length-1].story;
 if(next===0)return 0;
 const a=narrationTiming[next-1],b=narrationTiming[next];
 return a.story+(mediaTime-a.media)*(b.story-a.story)/(b.media-a.media);
}

const CAPTIONS:[number,string][]=[...gritScript.captions as [number,string][],[72,'In football, grit means returning to useful practice: one touch, feedback, and time to recover.']];
const CALLOUTS:[number,number,string][]=[
 [0,4.6,'A tree grows in two directions at the same time?'],[6.95,8.35,'UP'],[13.1,15.5,'ONE TREE. TWO FORCES.'],
 [27.18,29,'DARKNESS.'],[29,30.4,'RESISTANCE.'],[30.4,31.8,'PRESSURE.'],[31.8,33.2,'AND DIRT.'],[33.6,37.2,'DEEPER UNDERGROUND'],
 [37.5,43,'BREAK THROUGH'],[49.2,55.6,'THE WORK NOBODY SEES'],
 [59.2,66.4,'THE FRUIT OF YOUR WORK'],[69,72,'KEEP SHOWING UP'],
 [72,76.4,'SMALL PRACTICE. REAL GROWTH.']
];
export default function GritFilm({onClose,onFinish,embedded: _embedded=false,origin}:{onClose:()=>void;onFinish:()=>void;embedded?:boolean;origin?:{x:number;y:number;size:number;height:number;radius:string;color:string}}){
 ...
 const storySource=()=>{const ratio=window.innerWidth/window.innerHeight;return '/stories/films/grit/'+(ratio<.75?'portrait':ratio<1.34?'square':'landscape')+'.mp4?v=immersive-23-adam-recording';};
 ...
 const groundOffset=media&&media.videoWidth?65*.7*(media.videoWidth>media.videoHeight?.72:.82)*Math.min(media.videoWidth,media.videoHeight)/1000*Math.max(media.clientWidth/media.videoWidth,media.clientHeight/media.videoHeight):0;
 const callout=CALLOUTS.find(([from,to])=>time>=from&&time<to)?.[2];
 return <dialog ref={dialog} className={`${styles.dialog} ${entering?styles.entering:''} ${exiting?styles.exiting:''}`} aria-labelledby={uid} data-story-view data-story-complete={completed} onCancel={e=>{e.preventDefault();e.stopPropagation();dismiss();}}>
  ...
  <video ref={video} className={styles.film} preload="none" playsInline muted={muted} aria-label="Grit: roots before fruit" aria-describedby={uid+'-caption'} onTimeUpdate={e=>setTime(storyTime(e.currentTarget.currentTime))} onPlay={()=>{document.dispatchEvent(new CustomEvent('fi2-video-play',{detail:uid}));release.current??=holdVideoPlayback();setPlaying(true);}} onPause={()=>{setPlaying(false);stop();}} onEnded={()=>{setPlaying(false);setCompleted(true);stop();}} onError={()=>{setPlaying(false);setError(true);stop();}}/>
  <header className={styles.top}><h2 id={uid}>Grit</h2><DoneButton onDone={dismiss}/></header>
  {!started&&<picture className={styles.poster}><source media="(min-aspect-ratio: 3/4) and (max-aspect-ratio: 4/3)" srcSet="/stories/films/grit/poster-square.jpg?v=immersive-23-adam-recording"/><source media="(orientation: portrait)" srcSet="/stories/films/grit/poster-portrait.jpg?v=immersive-23-adam-recording"/><img src="/stories/films/grit/poster-landscape.jpg?v=immersive-23-adam-recording" alt=""/></picture>}
  {started&&callout&&<div className={`${styles.callout} ${time>=33.6&&time<37.2?styles.lowerCallout:''}`} aria-hidden="true">{callout}</div>}
  {started&&time>=9.1&&time<13&&<div className={styles.rootFruit} style={{top:`calc(50% + ${groundOffset}px)`}} aria-hidden="true">ROOTS MIRROR FRUIT</div>}
  {started&&time>=15.5&&time<23.5&&<div className={`${styles.forces} ${(time<18.6||time>=20.8)?styles.gravity:''}`} aria-hidden="true"><div><span>{time>=20.8?'INTO THE DARK':time<18.6?'GRAVITROPIC':'PHOTOTROPIC'}</span><span>{time>=20.8?'Roots keep growing down':time<18.6?'Toward gravity':'Toward light'}</span></div></div>}
  <div role="region" className={styles.bottom} aria-label="Story playback and captions">
   {started&&time>=5.14&&time<6.85&&<div className={styles.downCue} aria-hidden="true">DOWN</div>}
   <div className={styles.controls}>
    <button type="button" onClick={completed&&time>76?dismiss:toggle}><Icon name={completed&&time>76?'check':playing?'pause':'play'}/>{completed&&time>76?'Finish story':playing?'Pause':started?'Continue':'Play story'}</button>
   ...
   <div className={styles.captionArea}><p id={uid+'-caption'} className={styles.caption}>{completed&&time>76?'In football, grit means returning to small, useful practice. Work on one touch, ask for feedback, and make time to recover.':!started?'Roots before fruit. A story about the practice nobody sees.':([...CAPTIONS].reverse().find(([start])=>time>=start)?.[1])}</p></div>
