// Offline mix only: writes <story>/narration-music.m4a next to the dry narration.mp3 the riso track stories play; swap the story's audio.src to adopt it.
...
const duration=Number(execFileSync('ffprobe',['-v','error','-show_entries','format=duration','-of','csv=p=0',dry],{encoding:'utf8'}));
...
copyFileSync(mix,`${base}/narration-music.m4a`);
console.log(`Mixed ${story==='regulate'?'Ascension':'Wildflowers'} under ${story}, ${duration}s; wrote narration-music.m4a (dry narration.mp3 untouched).`);
