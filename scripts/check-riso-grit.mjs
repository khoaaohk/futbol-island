// Live-app check for the riso Grit story (TRACK mode: one narration file, a seek slider instead of chapter dots).
// Opens Paths → 9v9 → "Story: Not yet is a starting point." at four viewports and asserts the player contract, seeking to each
// visual chapter start for a screenshot. Copied from check-riso-films-browser.mjs (chapters mode) and adapted for track mode.
// Usage: node scripts/check-riso-grit.mjs [--out dir]
...
const title='Not yet is a starting point.',format='9v9',out=arg('--out','/private/tmp/claude-501/-Users-khoado-Desktop-Warp-Claude-Projects/a94b3f51-4163-4951-814b-79d6d6d74e9a/scratchpad/riso/grit');
const VISUAL_STARTS=[0,6.3,14.26,21.92,32.8,39.08,43.9,51.82,60.34],CAPTIONS=22;
...
try{for(const [width,height] of sizes){
 ...
 const film=page.locator('[data-riso-story]');await film.waitFor({timeout:20000});const id=await film.getAttribute('data-path-film');assert.equal(id,'grit','the riso grit story is the one that opened');
 ...
 assert.equal(await film.getByRole('button',{name:/Chapter \d:/}).count(),0,'track mode shows a seek slider, not chapter dots');
 assert.equal(await film.locator('input[type="range"]').count(),1,'one seek slider');
 // seek to each visual chapter start (+.6 s so the arrival scale has settled) through the player's dev controls, then screenshot
 for(let i=0;i<VISUAL_STARTS.length;i++){await film.evaluate((node,t)=>node.filmControls.seekTime(t),VISUAL_STARTS[i]+.6);await page.waitForTimeout(600);await page.screenshot({path:path.join(out,`app-${id}-${width}x${height}-ch${i+1}.png`)});}
 assert.equal(await film.getByRole('button',{name:'Pause',exact:true}).count(),1,'still playing after seeks');
 await film.evaluate(node=>node.filmControls.seekTime(27));await page.waitForTimeout(900);await page.screenshot({path:path.join(out,`app-${id}-${width}x${height}.png`)});
 ...
 await film.getByRole('button',{name:'Read story transcript',exact:true}).click();const transcript=film.getByRole('article',{name:'Story transcript'});await transcript.waitFor();assert.equal(await transcript.locator('p').count(),CAPTIONS,`transcript lists the ${CAPTIONS} captions`);
