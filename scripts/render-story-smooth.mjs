// Project extension: 24 unique samples/sec for smooth motion; upstream core remains unchanged.
...
const url = process.env.STORY_URL || pathToFileURL(path.resolve(file)).href + `?bare=1&frame=0&ar=${encodeURIComponent(ar)}` + (width ? `&w=${width}` : '');
...
try {
  ...
  page.on('request',request=>request.url().endsWith('/favicon.ico')?request.respond({status:204,body:''}):request.continue());
  page.on('pageerror', e => errors.push(String(e)));
  page.on('console', m => { if (m.type() === 'error') errors.push(m.text()); });
  await page.goto(url, {waitUntil: 'load'});
  await page.waitForFunction('window.__ready === true', {timeout: 120000});
  const N = await page.evaluate(() => window.__NDRAW*2), size = await page.evaluate(() => window.__size);
  ...
  for (const [k, i] of list.entries()) {
    try {
      save(path.join(frames, `${String(i).padStart(4, '0')}.png`), await page.evaluate(i => window.__frame(i/2), i));
    } catch (e) {
...
ff(['-framerate', '24', '-i', path.join(frames, '%04d.png'), '-r', '24', '-pix_fmt', 'yuv420p', '-crf', '18', mp4]);
// two tiles per second of film (every 6th drawn frame = every 12th output frame), 6 across
const rows = Math.ceil(total / 12 / 6);
