// Live-app check for the TRACK-mode riso story Love Futsl: opens Paths → Futsal → "Story: Smaller court. Bigger game." at four viewports,
// asserts the player contract (seek slider instead of chapter dots, 22 caption paragraphs), screenshots every visual scene via the dev seek hook.
// Usage: node scripts/check-riso-futsl.mjs [--title "Smaller court. Bigger game."] [--format Futsal] [--out dir]
...
const title=arg('--title','Smaller court. Bigger game.'),format=arg('--format','Futsal'),out=arg('--out','/private/tmp/claude-501/-Users-khoado-Desktop-Warp-Claude-Projects/a94b3f51-4163-4951-814b-79d6d6d74e9a/scratchpad/riso/futsl');
mkdirSync(out,{recursive:true});
const sizes=[[390,850],[320,568],[844,390],[1440,850]];
// visual scene starts (media time) and extra mid-scene sample times
const scenes=[0,5.36,16.56,23.30,31.04,38.20,46.26],extra=[4.4,7.2,13.0,19.0,24.6,29.6,33.6,36.9,42.0,44.6,48.0,52.5];
...
try{for(const [width,height] of sizes){
 ...
 assert.equal(await film.getByRole('button',{name:/Chapter \d:/}).count(),0,'track mode shows no chapter dots');
 assert.equal(await film.getByRole('slider',{name:'Story progress'}).count(),1,'track mode shows the seek slider');
 // pause, then scrub through every visual scene with the dev hook and screenshot each
 await film.getByRole('button',{name:'Pause',exact:true}).click();await page.waitForTimeout(150);
 const seek=async t=>{await film.evaluate((node,t)=>{node.filmControls.seekTime(t);},t);await page.waitForTimeout(160);};
 for(let i=0;i<scenes.length;i++){await seek(scenes[i]+.6);await page.screenshot({path:path.join(out,`app-${id}-${width}x${height}-ch${i+1}.png`)});}
 for(const t of extra){await seek(t);await page.screenshot({path:path.join(out,`app-${id}-${width}x${height}-t${t.toFixed(1)}.png`)});}
 await seek(24.6);await page.screenshot({path:path.join(out,`app-${id}-${width}x${height}.png`)});
 ...
 // resume and confirm the clock advances (media time drives the visuals)
 await film.getByRole('button',{name:'Continue',exact:true}).click();await page.waitForTimeout(900);const t1=Number(await canvas.getAttribute('data-time'));await page.waitForTimeout(700);assert.ok(Number(await canvas.getAttribute('data-time'))>t1,'playback advances on media time');
 await film.getByRole('button',{name:'Pause',exact:true}).click();await page.waitForTimeout(150);
 await film.getByRole('button',{name:'Read story transcript',exact:true}).click();const transcript=film.getByRole('article',{name:'Story transcript'});await transcript.waitFor();assert.equal(await transcript.locator('p').count(),22,'transcript lists the 22 captions');
