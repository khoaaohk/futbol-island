...
for(const mode of Object.keys(TRAVEL_MODES)){
 ...
 if(mode!=='walk'){
   ...
   for(const shoulder of shoulders){const elbow=shoulder.children.find(n=>n.type==='Group');const hand=player.root.worldToLocal(elbow.localToWorld(new (require('three').Vector3)(0,-.255,0)));assert.ok(Math.hypot(hand.y-(mode==='scooter'?1.25:1.13),hand.z-.48)<.025,'Rider hands reach handlebars');}
 }
}
